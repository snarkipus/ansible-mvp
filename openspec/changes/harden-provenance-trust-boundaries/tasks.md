...
- [x] 4.2 Create the versioned `controlled-source-demo-v0.1.2` bootstrap contract, without rewriting it in an existing repository or mutating older tags, and update defaults, compatibility checks, fixtures, and tests; run integrity remains bound to the admission-time resolved commit because Git tags can move.
