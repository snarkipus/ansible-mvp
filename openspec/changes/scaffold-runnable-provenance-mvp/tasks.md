...
- [ ] 1.1 Add root `.gitignore` entries for generated run outputs, generated reports, Python caches, and local virtual environments without ignoring source/config files needed for the MVP.
- [ ] 1.2 Add `pyproject.toml` managed by `uv` with the MVP runtime, test, lint, format, and type-check dependencies: PyYAML, pytest, ruff, mypy, openpyxl, python-pptx, and any minimal chart dependency selected for `chart.png`.
- [ ] 1.3 Create the repository structure for `ansible/`, `configs/`, `scripts/`, `src/provenance/`, `templates/`, `tests/`, and `runs/.gitkeep`.
...
- [ ] 2.1 Implement `make bootstrap-controlled-source` to create or verify a sibling `../controlled-source-demo` Git repository.
- [ ] 2.2 Populate the controlled source demo with tracked synthetic fixture inputs for `dirA`, `dirB`, and `dirC`.
...
- [ ] 3.5 Add `src/provenance/manifest.py` to assemble the run manifest from config, Git state, inventories, stages, logs, scheduler metadata, validations, products, and hash policy.
...
- [ ] 4.1 Add `configs/run.synthetic.yaml` and `configs/expected_shape.required_extract.yaml` with the documented synthetic run, controlled scripts, stage declarations, approved command paths, hash policy, layout, and validation expectations.
- [ ] 4.2 Add `ansible/inventory/localhost.ini`, group variables, and `ansible/playbooks/run_synthetic_workflow.yml` for local execution.
- [ ] 4.3 Implement preflight so missing repos, non-Git repos, unresolved refs, dirty controlled source state, dirty wrapper controlled paths, missing scripts, untracked scripts, unknown controlled script references, and uncontrolled stage command paths fail before execution.
- [ ] 4.4 Implement workspace preparation for `runs/{run_id}/sim-run-root/` and `runs/{run_id}/provenance/` without placing provenance files inside `sim-run-root/`.
...
- [ ] 5.1 Implement the synthetic simulation stage so controlled scripts write the raw output `sim-run-root/lists/dirC/sim-out.dat`.
- [ ] 5.2 Implement required extraction to produce `provenance/products/extracted/required.csv` from raw outputs.
...
- [ ] 7.5 Update README quickstart notes if implementation details differ from the existing documented command shape, and link to the handoff guide.
